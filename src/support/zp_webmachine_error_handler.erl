...
render_error(404, Req, _Reason) ->
    ...
    Context1  = zp_context:set_context([{error_code, 404},
                                        {error_dump, ErrorDump}], Context),
    Html      = zp_template:render("error.tpl", Context1),
    {Output, _Context} = zp_context:output(Html, Context1),
    Output;
...
render_error(500, Req, Reason) ->
    ...
    Context   = zp_context:new(),
    Context1  = zp_context:set_context([{error_code, 500},
                                        {error_dump, ErrorDump}], Context),
    Html      = zp_template:render("error.tpl", Context1),
    {Output, _Context} = zp_context:output(Html, Context1),
    Output.
