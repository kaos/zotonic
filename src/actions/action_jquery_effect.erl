...
render_action(_TriggerId, TargetId, Args, Context) ->
	...
	Speed   = proplists:get_value(speed, Args, "def"),
	...
	Script = case Type of
	    'show'   when Speed=="def" -> "show();";
		'hide'   when Speed=="def" -> "hide();";
		'toggle' when Speed=="def" -> "toggle();";
		'show'          -> io_lib:format("show(~p);", [Speed]);
		'hide'          -> io_lib:format("hide(~p);", [Speed]);
		'toggle'        -> io_lib:format("toggle(~p);", [Speed]);
		'add_class'     -> io_lib:format("addClass('~s');", [Class]);
		'remove_class'  -> io_lib:format("removeClass('~s');", [Class]);
		'appear'        -> io_lib:format("fadeIn(~p);", [Speed]);
		'fade'          -> io_lib:format("fadeOut(~p);", [Speed]);
