...
render_action(TriggerId, TargetId, Args, Context) -> 
	...
	{PostbackMsgJS, _PickledPostback} = z_render:make_postback({shop_select_variant, Id}, postback, TriggerId, TargetId, ?MODULE, Context),
	{PostbackMsgJS, Context}.

event({postback, {shop_select_variant, Id}, _TriggerId, _TargetId}, Context) ->
    Variant = z_convert:to_binary(z_context:get_q("triggervalue", Context)),
    ...
    Html = z_template:render("_product_price.tpl", Vars, Context),
    Context1 = z_render:update("product-price", Html, Context),
    ...
    case Props of
        ...
        _ ->
            ...
            {ok, Tag} = z_media_tag:tag(Filename, [{alt,""}, {crop,true}, {width,300}]),
            z_render:update("variant-image", Tag, Context1)
    end.
