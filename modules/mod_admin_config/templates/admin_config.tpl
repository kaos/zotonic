...
{% with q.qcat|eq:"event" as is_event %}
	...
	<div id="content" class="zp-100">
		<div class="block clearfix">
			...
			<div class="clearfix">
				{% button class="" text="new config setting" action={dialog_config_new on_success={reload}} %}
			</div>

			<h3 class="above-list ">Configurations overview</h3>
			<ul class="short-list">
				<li class="headers clearfix">
					<span class="zp-10">Module</span>
					<span class="zp-20">Key</span>
					<span class="zp-45">Value</span>
					...
					{% with c.id as id %}
						<li id="{{ #li.id }}">
							<a id="{{ #a.id }}" href="#edit-config" class="clearfix">
								<span class="zp-10">{{ module|escape|default:"-" }}</span>
								<span class="zp-20">{{ key|escape|default:"-" }}</span>
								<span class="zp-45">{{ c.value|escape|default:"-" }}</span>
								<span class="zp-15">{{ c.modified|date:"d M Y, H:i" }}</span>
								<span class="zp-10">
									...
									{% button text="edit &raquo;" action={dialog_config_edit module=module key=key on_success={reload}} %}
								</span>
							</a>
